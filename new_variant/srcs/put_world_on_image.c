...
static void		put_floor_and_ceil_on_img(t_pfac_arg *arg)
{
	...
	while (y < WIN_H)
	{
		...
		tex = match_tex(arg->texs,
			arg->map->coord[(int)tmp[Y]][(int)tmp[X]], FLOOR_CEIL);
		...
		SET_PIXEL((*(arg->img)), arg->vslice.x, (WIN_H - y),
			GET_PIXEL((tex + 1)->img, (int)pos_in_tex[X], (int)pos_in_tex[Y]));
		y++;
	}
}
...
void			put_world_on_image(t_game *game, t_texture texs[], t_image *img)
{
	...
	while (x < WIN_W)
	{
		...
		// printf("m_x: %d m_y: %d\n", ray.end_on_map[X], ray.end_on_map[Y]);
		put_wall_on_img(vslice, x_in_tex(&ray),
			match_tex(texs, ray.value, ray.contact_side), img);
		put_floor_and_ceil_on_img(&(t_pfac_arg){vslice, curr_angle, &ray, &game->map,
			texs, img});
