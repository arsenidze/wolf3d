...
/*   Updated: 2018/03/25 18:51:45 by amelihov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
short		graphics_init(t_graphics *graphics)
{
	...
	if (!(graphics->win = mlx_new_window(graphics->mlx, WIN_W, WIN_H,
		PROGNAME)))
	{
	...
	graphics->img.data = mlx_get_data_addr(graphics->img.ptr,
		&graphics->img.bpp, &graphics->img.sl, &graphics->img.endian);
	graphics->img.bpp /= 8;
	return (SUCCESS);
}
